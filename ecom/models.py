...
class UserManager(BaseUserManager):
    def create_user(self, email, name, phone, password=None, gender="Other", role=UserRole.CUSTOMER):
        if not email:
        ...
        user = self.model(email=email, name=name, phone=phone, gender=gender, role=role)
        user.set_password(password)  # Hash password
        user.save(using=self._db)
        return user

    def create_superuser(self, email, name, phone, password=None):
        user = self.create_user(email, name, phone, password, role=UserRole.ADMIN)
        user.is_staff = True
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    # groups = models.ManyToManyField(Group, related_name="ecom_user_groups", blank=True)
    user_permissions = models.ManyToManyField(Permission, related_name="ecom_user_permissions", blank=True)
